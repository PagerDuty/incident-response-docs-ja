...
!!! note "資料"独自の内部ドキュメントについては、このページに必要な情報がすべて顕著に表示されていることを確認してください。たとえば、電話会議の番号、Slackチャンネル、重要なチャットコマンドなどです。次に例を示します。
...
1. インシデントコールとチャットに参加します(上記のリンクを参照)。
    ...
    * しかし、積極関与を希望する場合は、両方に参加する必要があります。何らかの理由でコールに参加できない場合は、専属代理が必要です。チャットルームでの分裂した話し合いは、最終的にはまとまりがなくなることがほとんどです。
...
## インシデントコマンダーの手順
できるだけ早く、できるだけ安全に、インシデントを解決し、副担当者を使って支援してください。貴社の裁量により、関連する専門家にタスクを委任します。
...
1. 調査&アンプ、修理アクション(ロールバック、レートリミットサービスなど)を特定し、アクションを関連サービスエキスパートに委任します。典型的にはこのようなもの(明らかに完全なリストではない)で、
    * **デプロイメント不良:**ロールバックします。
    * **Web アプリケーションのスタック/クラッシュ:**ローリング再起動を実行します。
    * **イベントフラッド:**自動スロットリングで十分であることを確認し、十分でない場合は手動で調整します。
    * **データセンターの停止:**自動化により、不良データセンターが削除されたことを確認します。そうでない場合は、強制的に実行します。
